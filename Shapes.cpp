...
////////////////////////////////////////
// Реализация методов класса CShape

CShape::CShape(int x, int y, WORD s) : CBasePoint(x, y, s)
{
	m_wSize = s;
}

CShape::CShape() : CBasePoint()
{
	m_wSize = 40;
}
...
void CShape::Show(CDC* pDC)
{
	...
	CRect rect1(x - s, y - 0.2*s, x + s, y + 0.2*s);
	pDC->FillRect(rect1, new CBrush(RGB(255, 0, 0)));
	CRect rect2(x - 0.2*s, y - s, x + 0.2*s, y + s);
	pDC->FillRect(rect2, new CBrush(RGB(255, 0, 0)));
	// Восстанавливаем контекст
	RestoreDC(pDC);
}

void CShape::GetRegion(CRgn& Rgn)
{
	int s = m_wSize / 2;
	Rgn.CreateRectRgn(x - s, y - s, x + s, y + s);
}
...
/////////////////////////////////////
// методы класса стрелка

CArrow::CArrow(): CPolygon()
{
	m_wSize=0;
	m_bPolygon=FALSE;
} 
...
IMPLEMENT_SERIAL(CArrow, CObject, 1)
...
void CArrow::Show(CDC* pDC)
{
	// Устанавливаем перео и кисть
	PrepareDC(pDC);
	//m_PointsArray.Add(CPoint(m_PointsArray[0].x + 200, m_PointsArray[0].y + 300));
	pDC->Polygon(m_PointsArray.GetData(), m_PointsArray.GetSize());
	// Восстанавливаем контекст
	RestoreDC(pDC);
} 

void CArrow::SetCenter(int x, int y, WORD s) 
{
	m_wSize = s;
	m_PointsArray.Add(CPoint(x + s, y + 2 * s));
	m_PointsArray.Add(CPoint(x + 4*s, y + 2*s));
	m_PointsArray.Add(CPoint(x, y + 6*s));
	m_PointsArray.Add(CPoint(x - 4*s, y + 2*s));
	m_PointsArray.Add(CPoint(x - s, y + 2*s));
	m_PointsArray.Add(CPoint(x - s, y - 2*s));
	m_PointsArray.Add(CPoint(x - 4*s, y - 2*s));
...
void CArrow::GetRegion(CRgn &Rgn)
{
	Rgn.CreatePolygonRgn(m_PointsArray.GetData(), m_PointsArray.GetSize(), ALTERNATE);
}

void CArrow::Transform(const CPoint &point0, double ang, int a, int b)
{
	for(int i=0; i<m_PointsArray.GetSize(); i++)
		m_PointsArray[i]=::Transform(m_PointsArray[i], m_PointsArray[0], ang, a, b);
};
...
void CBezier::ShowBaseline(CDC* pDC)
{
	// Устанавливаем перео и кисть
	PrepareDC(pDC);
	// Рисуем 
...
void CBezier::ShowSpline(CDC* pDC)
{
	int nCount = m_PointsArray.GetSize();

	// Сплайн рисуем красным пером
	CPen PenSpline; PenSpline.CreatePen(PS_SOLID, 50, RGB(255, 0, 0));
	CPen* pPenOld = pDC->SelectObject(&PenSpline);
	// Рисуем кривую Безье методом класса CDC::PolyBezier
	pDC->PolyBezier(m_PointsArray.GetData(), nCount / 3 * 3 + 1);
	// Покажем точки стыковки сегментов
	for (int i = 3; i < nCount; i += 3)
		pDC->Ellipse(m_PointsArray[i].x - 4, m_PointsArray[i].y - 4,
			m_PointsArray[i].x + 4, m_PointsArray[i].y + 4);
...
CBezierStar::CBezierStar() : CPolygon()
{
	m_wSize = 0;
	m_bPolygon = FALSE;
}

CBezierStar::~CBezierStar()
{
	m_PointsArray.RemoveAll();
}


IMPLEMENT_SERIAL(CBezierStar, CObject, 1)
void CBezierStar::Serialize(CArchive& ar)
{
	if (ar.IsStoring()) // сохранение
	{
		// Сохраняем параметры объекта
...
void CBezierStar::SetBasePoints(int x, int y, WORD s)
{
	m_wSize = s;
	
	m_PointsArray.Add(CPoint(x + 0.5 * s, y + 4 * s));

	m_PointsArray.Add(CPoint(x, y + 4.5 * s)); //
	m_PointsArray.Add(CPoint(x, y + 4.5 * s)); //

	m_PointsArray.Add(CPoint(x - 0.5 * s, y + 4 * s));
	m_PointsArray.Add(CPoint(x - s, y + 3.5 * s)); //
	m_PointsArray.Add(CPoint(x - 0.5 * s, y + 1.5 * s)); // 
	m_PointsArray.Add(CPoint(x - s, y + s));
	m_PointsArray.Add(CPoint(x - 1.5 * s, y + 0.5 * s)); // 
	m_PointsArray.Add(CPoint(x - 3.5 * s, y + s)); //
	m_PointsArray.Add(CPoint(x - 4 * s, y + 0.5 * s));

	m_PointsArray.Add(CPoint(x - 4.5 * s, y)); // 
	m_PointsArray.Add(CPoint(x - 4.5 * s, y)); //
...
void CBezierStar::ShowBaseline(CDC* pDC)
{
	// Устанавливаем перео и кисть
	PrepareDC(pDC);
	// Рисуем 
...
void CBezierStar::ShowSpline(CDC* pDC)
{
	int nCount = m_PointsArray.GetSize();
...
void CBezierStar::Show(CDC* pDC)
{
	//ShowBaseline(pDC);
	ShowSpline(pDC);
}

void CBezierStar::GetRegion(CRgn& Rgn)
{
	Rgn.CreatePolygonRgn(m_PointsArray.GetData(), m_PointsArray.GetSize(), ALTERNATE);
}
...
CPic::CPic() : CBasePoint()
{
	m_wSize = 100;
	m_bmp.LoadBitmap(IDB_BITMAP1);
}

CPic::CPic(int x, int y, WORD s) : CBasePoint(x, y, s)
{
	m_wSize = s;
	m_bmp.LoadBitmap(IDB_BITMAP1);
}
...
void CPic::Show(CDC* pDC)
{
	int s = m_wSize;
	PrepareDC(pDC);
	pDC->Rectangle(x, y, x + s, y - s);
	CDC cDC;
	cDC.CreateCompatibleDC(pDC);
	cDC.SelectObject(m_bmp);
	pDC->SetStretchBltMode(WHITEONBLACK);
	pDC->StretchBlt(x, y, s, -s, &cDC, 0, 0, 369, 370, SRCCOPY);
	//раскомментить для лабы 5
	//pDC->SetStretchBltMode(BLACKONWHITE);
	//pDC->StretchBlt(x, y+s, s, -s, &cDC, 0, 0, 369, 370, SRCCOPY); 
	RestoreDC(pDC);
}


void CPic::GetRegion(CRgn& Rgn)
{
	int s = m_wSize / 2;
	Rgn.CreateRectRgn(x - s, y - s, x + s, y + s);
}
