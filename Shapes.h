...
class CShape : public CBasePoint
{
...
// класс Стрелка
class CArrow : public CPolygon
{
	DECLARE_SERIAL(CArrow)
	BOOL	m_bPolygon = TRUE;
...
public:
	...
	void ShowBaseline(CDC* pDC);
	void ShowSpline(CDC* pDC);
	void Show(CDC* pDC);
	void GetRegion(CRgn& Rgn);

};

class CBezierStar : public CPolygon
{
	DECLARE_SERIAL(CBezierStar)
	BOOL	m_bPolygon = FALSE;
...
public:
	...
	void SetBasePoints(int x, int y, WORD s);
	void ShowBaseline(CDC* pDC);
	void ShowSpline(CDC* pDC);
	void Show(CDC* pDC);
	void GetRegion(CRgn& Rgn);
